...
static void initialize_ili9341()
{
    ...
    const esp_lcd_panel_dev_config_t panel_config = {.reset_gpio_num = GPIO_LCD_RST,
#if ESP_IDF_VERSION < ESP_IDF_VERSION_VAL(5, 0, 0)
                                                     .color_space = ESP_LCD_COLOR_SPACE_RGB,
#else
                                                     .rgb_endian =
                                                         LCD_RGB_ENDIAN_BGR, // Some ili9341s have RGB = 0, others RGB = 1. Mine is the latter
#endif
...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        {
        case 'd':
            ESP_ERROR_CHECK(esp_lcd_panel_draw_bitmap(panel_handle, 0, 0, LCD_H_RES, LCD_V_RES, color_buffer));

...
static void oled_flush_framebuffer(void)
{
    ...
    esp_err_t err = esp_lcd_panel_draw_bitmap(s_oled_panel, 0, 0, OLED_WIDTH, OLED_HEIGHT, s_oled_framebuffer);
    if (err != ESP_OK)
    {
...
static void oled_plot_segment(uint64_t *cursor, uint64_t total_time, uint32_t duration, uint32_t level, int *last_y)
{
...
static void oled_render_capture(void)
{
    ...
    for (size_t i = 0; i < s_capture_count; ++i)
    {
        oled_plot_segment(&cursor, total_time, s_capture_buffer[i].duration0, s_capture_buffer[i].level0, &last_y);
        oled_plot_segment(&cursor, total_time, s_capture_buffer[i].duration1, s_capture_buffer[i].level1, &last_y);
    }
    if (cursor < total_time)
    {
        oled_plot_segment(&cursor, total_time, (uint32_t)(total_time - cursor), 0, &last_y);
    }
    oled_flush_framebuffer();
}
...
static bool IRAM_ATTR rmt_rx_done_callback(rmt_channel_handle_t channel, const rmt_rx_done_event_data_t *edata,
                                           void *user_data)
{
    (void)channel;
...
static void capture_for_duration(uint32_t duration_ms)
{
    ...
    ESP_ERROR_CHECK(rmt_receive(s_rx_channel, s_rx_raw_symbols, sizeof(s_rx_raw_symbols), &s_rx_receive_config));
    ...
    while (keep_rxing || rx_inflight)
    {
        ...
        if (xQueueReceive(s_rx_event_queue, &event, wait_ticks) == pdTRUE)
        {
            ...
            {
                ESP_ERROR_CHECK(
                    rmt_receive(s_rx_channel, s_rx_raw_symbols, sizeof(s_rx_raw_symbols), &s_rx_receive_config));
                rx_inflight = true;
            }
        }
